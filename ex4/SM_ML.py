...
from utils import diversity_order_from_ser_snr_curve, generate_n_qubit_combinations
...
class SM_ML:
    ...
    def __init__(self, SNR_dB, N_Tx=3, num_of_symbols=50000):
        self.SNRs_dB = SNR_dB
    ...
    def exec(self, plot=False):
        ...
        # Create a matrix of possible combinations for ML
        CombinationLoc = generate_n_qubit_combinations(self.N_Tx)
        ...
        for k in range(len(self.SNRs_dB)):
            ...
            s_tilde_ml = np.zeros((self.N_Tx,self.N), dtype=complex)
            #s_hat = np.zeros(self.N, dtype=complex)
            rho = 1 / np.sqrt(self.SNRs[k])

            # Signal Generation
            s = self.QPSK_vec[np.random.randint(0, 4, (self.N_Tx,self.N))]
            ...
            for kk in range(self.N):
                ...
                DiffVec = np.tile(y[:, np.newaxis], (1, 4 ** self.N_Tx)) - (1 / np.sqrt(2)) * H @ CombinationMat
        ...
        if plot:
            ...
            plt.xlabel('SNR (dB)')
            plt.ylabel('SER')
            plt.title(f'SM {self.N_Tx}x2 Rayleigh - Maximum Likelihood\n \
...
if __name__ == "__main__":
    ...
    sim = SM_ML(SNR_dB=np.arange(args.min_snr,args.max_snr,1), N_Tx=args.n_tx)
